"""Complete Vision System - Element detection + Green boxes"""
...
class Vision:
            ...
            document.querySelectorAll(selectors).forEach(el => {
                try {
                    ...
                    const inViewport = (
                        ...
                        rect.bottom > -300 && rect.left < window.innerWidth + 100 &&
                        rect.right > -100
                    );
    ...
    def create_labeled_screenshot(self, page: Page, elements: List[Dict] = None) -> Tuple[bytes, str]:
        ...
        try:
            ...
            for elem in elements:
                ...
                colors = {'input': (52, 152, 219), 'button': (46, 204, 113), 'a': (155, 89, 182), 'range': (231, 76, 60)}
                color = colors.get(elem['tag'], (243, 156, 18))
                
                box = [elem['left'], elem['top'], elem['left'] + elem['width'], elem['top'] + elem['height']]
                draw.rectangle(box, outline=color, width=3)
                
                label = f"[{elem['id']}]"
                label_pos = (elem['left'] + 5, max(5, elem['top'] - 20))
                bbox = draw.textbbox((0, 0), label, font=font)
                bg_box = [label_pos[0] - 2, label_pos[1] - 2, label_pos[0] + (bbox[2] - bbox[0]) + 2, label_pos[1] + (bbox[3] - bbox[1]) + 2]
    ...
    def extract_page_content(self, page: Page) -> Dict:
        ...
        () => {
            ...
            const productSelectors = ['[data-testid*="product"]', '.product-card', 'article', '[class*="ProductCard"]'].join(',');
            ...
            document.querySelectorAll(productSelectors).forEach((card, i) => {
                ...
                if (link) {
                    const title = (card.querySelector('h1,h2,h3,h4')?.innerText || link.innerText).trim();
                    data.products.push({
