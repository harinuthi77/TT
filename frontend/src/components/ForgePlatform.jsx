...
import { Send, Upload, Mic, Code, Globe, Image, Database, Zap, Play, Pause, Download, Eye, Hammer, Menu, X, Search, BookOpen, Layers, Grid, FolderKanban, MessageSquarePlus, Settings, Trash2, ChevronDown, ChevronRight, Folder } from 'lucide-react'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Steps */}
          {[
            { step: 'Analyzing blueprint', status: 'done' },
            { step: 'Heating the forge', status: 'done' },
            { step: 'Crafting solution', status: 'active' },
            { step: 'Tempering code', status: 'pending' },
            { step: 'Polishing output', status: 'pending' }
          ].map((item, i) => (
            <div
              key={i}
              style={{
            ...
            >
              <div style={{
                ...
                borderRadius: '50%',
                border: `2px solid ${
                  item.status === 'done' ? '#ff8a00' :
                  item.status === 'active' ? '#ff8a00' : '#2a2a2a'
                }`,
                background: item.status === 'done' ? '#ff8a00' : 'transparent',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                boxShadow: item.status === 'active' ? '0 0 12px rgba(255, 138, 0, 0.5)' : 'none'
              }}>
                {item.status === 'active' && (
                  <div style={{
                    width: '6px',
              ...
              <div style={{
                fontSize: '13px',
                color: item.status === 'pending' ? '#444444' : '#ffffff',
        ...
        <div style={{
          ...
          <div style={{
            position: 'absolute',
            inset: 0,
            background: 'radial-gradient(circle at center, rgba(255, 138, 0, 0.05) 0%, transparent 70%)',
        ...
        {showArtifact && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ color: '#ff8a00', marginBottom: '12px' }}>// Forged Output</div>
              <div style={{ color: '#cccccc' }}>
                {`function App() {
  return (
    <div className="forge">
      <h1>Built by FORGE</h1>
    </div>
  )
}`}
              </div>
